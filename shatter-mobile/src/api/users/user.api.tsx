...
export async function UserLoginApi(email: string, password: string): Promise<UserLoginResponse> {
    try{
...
export async function UserSignupApi(name: string, email: string, password: string): Promise<UserSignupResponse> {
    try{
...
export async function UserFetchApi(userId: string, token: string): Promise<UserDataResponse> {
    try{
...
export async function GetUserEventsApi(userId: string, token: string): Promise<GetUserEventsResponse> {
    try{
        ...
        if (err.response) {
            switch (err.response.status) {
                ...
                throw new Error("Server error. Please try again later.");
                default:
                throw new Error("Events cannot be loaded right now. Please try again later.");
            }
        }
...
export async function UserUpdateApi(userId: string, updates: Partial<User>, token: string): Promise<UserInfoUpdateResponse> {
    try{
        ...
        if (err.response) {
            switch (err.response.status) {
                case 403:
                throw new Error("You can only update your own profile. Please re-login and try again.");
                case 400:
                throw new Error("Invalid information entered. Please adjust your info and try again.");
                case 404:
