import { GetUserEventsApi, GetEventByCodeApi, GetEventByIdApi, JoinEventByIdGuestApi, JoinEventByIdUserApi } from "../api/events/event.api";
...
export async function getUserEvents(userId: string): Promise<UserEventsResponse | undefined> {
    const events = await GetUserEventsApi(userId);
    return events;
}

export async function JoinEventIdUser(eventId: string, userId: string, name: string): Promise<EventJoinIdResponse | undefined> {
    const response = await JoinEventByIdUserApi(eventId, userId, name);
    return response;
}

export async function JoinEventIdGuest(eventId: string, userId: string, name: string): Promise<EventJoinIdResponse | undefined> {
    const response = await JoinEventByIdGuestApi(eventId, userId, name);
    return response;
}